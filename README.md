...
zip 또는 cbz로 저장해둔 웹툰 회차를 폴더 기반으로 인식해서, 세로 스크롤로 읽을 수 있게 해주는
...
- 폴더 기반 라이브러리 스캔 (플랫폼 폴더 → 시리즈 폴더 → 회차 zip/cbz). 시리즈 폴더가
  작가별 폴더 등으로 한 단계 더 묶여 있어도(플랫폼 폴더 → 작가 폴더 → 시리즈 폴더)
  zip이 직접 있는 폴더를 자동으로 찾아 시리즈로 인식합니다.
- 폴더를 발견하는 즉시 그 자리에서 스캔해서 반영 — 전체 폴더 구조를 다 훑을 때까지
  기다리지 않고, 끝난 것부터 바로바로 목록에 나타납니다. 로컬 폴더가 항상 먼저,
  네트워크 드라이브는 나중에 스캔됩니다(`SLOW_PLATFORMS` 참고).
- 시리즈 목록 그리드 UI + 검색 + 정렬(최근 업데이트순 / 안읽은 회차 많은순 / 제목순).
  읽음 상태 필터(전체/읽지않음/읽는중/완독)와 플랫폼 필터(여러 개 동시 선택 가능)를
  같이 적용 가능. 검색창/필터 탭은 스크롤해도 화면 상단에 계속 고정됩니다.
...
- 리더 키보드 조작: ↑/↓(+볼륨키)는 누르고 있는 동안 계속 스크롤, ←/→와 Page Up/Down은
  한 번 누를 때마다 화면 한 칸(90%, 10%는 겹치게)씩 이동, 스페이스바는 자동 스크롤
  켜기/끄기(속도는 아래 스크롤 속도 설정을 그대로 따름, 켜짐 상태는 저장되지 않고
  리더를 열 때마다 항상 꺼진 채로 시작)
- 전체 화면 모드 + 확대/축소 슬라이더(50~100%, 원본보다 확대되지는 않음). 전체
  화면/스크롤 속도/확대축소 값을 웹툰마다 따로 기억할지, 전체 공통으로 적용할지
  목록 화면 설정 패널에서 선택 가능
- info.xml(ComicInfo 포맷)이 있는 플랫폼은 작가/그림/장르/연령등급/줄거리/원작
  정보를 리더 사이드바와 목록 화면 정보 패널에 표시
- 리더 사이드 패널: 전체 회차 목록 + 현재 보고 있는 회차 + 회차별 읽음/읽는 중 표시
- 목록 화면에서도 회차 목록을 바로 펼쳐볼 수 있는 사이드바 (페이지 이동 없이), 회차를
  탭하면 그 자리에서 바로 읽음/안읽음 토글(이어보기 위치도 같이 맞춰짐)
- 회차 단위 읽음/안읽음 수동 표시 (전체 또는 특정 회차 기준으로, 그 회차부터 포함해서 처리)
- 읽음 진행률은 SQLite에 저장되어 여러 기기에서 접속해도 이어보기 가능
- 라이브러리 자동/수동 재스캔 + 마지막으로 스캔한 시각 표시. 아직 스캔 안 끝난
  플랫폼도 폴더 구조만으로 먼저 필터 탭에 표시되고, 몇 초 간격으로 자동 확인해서
  스캔이 끝나는 대로 저절로 채워짐(수동 새로고침 불필요)
- 웹 UI에서 시리즈 폴더 스캔 제외/재포함 — 전체 재스캔 없이 그 폴더만 즉시 반영되어
  빠름. 제외해도 실제 폴더/zip 파일은 삭제하지 않음
- 네트워크 드라이브(원드라이브 등)의 파일 읽기는 로컬과 완전히 격리된 전용
  스레드풀에서 처리 — 네트워크 쪽이 응답 없이 멈춰도 로컬 웹툰 열람에는 영향 없음
- 커버/페이지 이미지는 브라우저에 캐싱되어, 리더를 갔다 와도 다시 내려받지 않음
...
시리즈 폴더가 플랫폼 폴더 바로 아래(1단계)에 있어야 하는 건 아닙니다. zip이 직접 든
폴더를 몇 단계 깊이든 찾아내므로, 아래처럼 작가별로 한 번 더 묶어둔 구조도 그대로
인식됩니다:
...
컨테이너 안 /library
 └ gdrive/
     ├ 01 작가/
     │   └ 어떤작가/
     │       └ 그 작가의 웹툰 제목/    ← 이것도 시리즈로 인식됨 (화면엔 "그 작가의 웹툰 제목"만 표시)
     │           └ 001 1화.zip
     └ 02 시리즈/
         └ 또 다른 웹툰 제목/
             └ 001 1화.zip
...
전부 선택 사항이며, 안 넣으면 아래 기본값으로 동작합니다.
...
- **스캔 중인 시리즈 폴더**: 플랫폼 폴더 안에서 실제로 스캔되고 있는 웹툰 폴더 목록.
  ...
  전체를 다시 스캔하지 않고 그 폴더 하나만 즉시 처리되어 빠릅니다. 제외해도 실제
  폴더나 zip 파일은 절대 지워지지 않고, 목록에서만 빠집니다.
- **제외된 폴더**: 한 번 제외했던 폴더 목록. "다시 추가"를 누르면 그 폴더 하나만
  즉시 다시 스캔되어 시리즈 목록에 반영됩니다. 방금 옮겨진 항목은 눈에 잘 띄도록
  목록 맨 위에 표시됩니다.
...
### 로컬을 먼저, 클라우드는 나중에 스캔하게 하기

`environment`에 `SLOW_PLATFORMS=<태그명>`을 추가하면(위 예시에서는 `gdrive`), 이
플랫폼은 항상 로컬 플랫폼들이 다 스캔된 뒤에 스캔됩니다. 파일 읽기(회차 열람, 커버
등)도 로컬과 완전히 분리된 전용 스레드풀에서 처리되어, 네트워크 드라이브가 느리거나
일시적으로 응답이 없어도 로컬 웹툰 열람에는 영향이 없습니다. 여러 개면 콤마로
구분해서 적으면 됩니다: `SLOW_PLATFORMS=gdrive,onedrive`.
