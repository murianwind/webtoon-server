...
- 다음 화 맨 앞부분이 이전 화 끝부분과 겹치는(리캡) 페이지를 자동으로 감지해서 건너뜀 —
  건너뛴 지점에는 눈에 잘 안 띄는 작은 버튼이 남아있어 원하면 펼쳐볼 수 있음. 실제로
  읽는 화는 항상 그 자리에서 계산되고(끄더라도 이 부분은 항상 동작), 재스캔 직후
  미리 계산해두는 배치 작업만 목록 화면 설정 패널에서 끄고 켤 수 있음(라이브러리가
  아주 크면 이 배치 작업이 오래 걸릴 수 있어서 켜고 끄는 선택지를 둠)
...
- 목록 화면에서도 회차 목록을 바로 펼쳐볼 수 있는 사이드바 (페이지 이동 없이), 회차를
  탭하면 리더에서 그 회차까지 읽었을 때와 똑같이 처리됨(그 회차까지 전부 읽음/그
  회차부터 전부 안읽음, 이어보기 위치도 같이 맞춰짐) — 회차 하나만 콕 집어 바꾸는
  게 아니라 범위로 처리됨
...
- 읽음 진행률 + 검색/정렬/필터 설정 + 라이브러리 등록 상태 + 공유 프로필 전체
  (허용 시리즈/둘러보기 필터/진행률/설정/접속 시간대/요청 내역/등록 기기)를
  통째로 백업/복원
- PWA 아이콘 지원 (아이폰/안드로이드 홈 화면에 추가 가능). 목록 화면 맨 위에서
  아래로 당기면(pull-to-refresh) 페이지가 새로고침됨
...
- **관리자 비밀번호**: **서버가 켜질 때마다 매번 새로 생성**되어 컨테이너 로그(`docker logs webtoon-server`)에 출력됩니다. `DISCORD_WEBHOOK_URL`을 설정해두면 디스코드로도 같이 전달됩니다. **이미 로그인해서 기억된 기기는 비밀번호가 바뀌어도 전혀 영향이 없습니다** — 그 기기들은 비밀번호가 아니라 기기 자신의 쿠키로 계속 통과되기 때문입니다. 즉 이 비밀번호는 "지금부터 새로 접속하려는 기기"를 등록할 때만 필요한 값이고, 컨테이너를 재시작하면 로그에서 최신 값을 다시 확인하면 됩니다(별도의 초기화 절차가 필요 없음).
- **공유 프로필 링크**(`/p/<토큰>/...`)는 관리자 비밀번호와 무관하게, 그 링크 자체가 접근 권한입니다. 프로필마다 다음을 관리자 화면에서 설정할 수 있습니다:
  - 이름 / 허용된 시리즈 / 둘러보기 연령 필터
  - **리더 접속 가능 시간대**(요일+시간): 설정한 시간이 아니면 실제로 회차를 읽는 것만 막히고, 목록 구경이나 이미 읽던 회차를 마무리하는 건 시간과 무관하게 계속 가능합니다
  - **접속 기기 2대 제한**: 프로필마다 최대 2대까지만 자동으로 접속되고, 3번째 기기가 들어오면 화면에 "승인 요청" 버튼이 나타나 관리자에게 디스코드로 알림이 갑니다. 승인하면 가장 오래 안 쓴 기기가 자동으로 밀려나고 새 기기가 등록됩니다. 관리자가 거부하면 그 기기는 처음과 같은 "신청" 화면으로 돌아갑니다.
- 프로필이 아직 허용받지 못한 시리즈에 "보여주세요"를 요청하면, 관리자에게 디스코드 알림이 가고 관리자 화면에서 승인/거부할 수 있습니다.
...
- **스캔 중인 시리즈 폴더**: 플랫폼 폴더 안에서 실제로 스캔되고 있는 웹툰 폴더 목록.
  웹툰이 아닌 폴더가 섞여 들어와 있으면 "제외"를 눌러 관리자 메인 화면에서 뺄 수
  있습니다. 전체를 다시 스캔하지 않고 그 폴더 하나만 즉시 처리되어 빠릅니다. 제외해도
  실제 폴더나 zip 파일은 절대 지워지지 않고, **데이터 자체도 남아있어서 공유
  프로필에게는 여전히 선택 후보로 줄 수 있습니다**(제외는 "관리자 자신의 메인
  화면"에서만 숨기는 것이라, 가족 프로필한테 특정 웹툰을 보여주고 싶지만 본인 목록
  에서는 안 보고 싶을 때도 쓸 수 있습니다). 스캔이 도중에 실패해도 이미 알고 있던
  폴더 목록은 그대로 남아 계속 제외/재포함 조작이 가능합니다.
- **제외된 폴더**: 한 번 제외했던 폴더 목록. "다시 추가"를 누르면 그 폴더 하나만
  즉시 다시 스캔되어 시리즈 목록에 반영됩니다. 방금 옮겨진 항목은 눈에 잘 띄도록
  목록 맨 위에 표시됩니다.
- **백업/복원**: 읽음 진행률, 검색/정렬/필터 설정, 제외된 폴더 목록, 그리고
  공유 프로필 전체(프로필 자체·공유 링크 토큰·허용 시리즈·둘러보기 필터·진행률·
  설정·접속 시간대·요청 내역·등록 기기)를 JSON 파일 하나로 내려받거나, 그 파일을
  다시 올려서 통째로 되돌릴 수 있습니다. 복원해도 프로필 공유 링크(토큰)는 그대로
  유지되어, 가족들에게 나눠준 링크를 다시 보낼 필요가 없습니다. 복원은 현재 저장된
  값을 덮어쓰므로, 실행 전에 확인 창이 한 번 뜹니다. (관리자 비밀번호 자체는 재시작
  마다 새로 만드는 값이라 백업에 포함되지 않습니다.)
...
- (Docker Desktop 사용자 한정) Docker Desktop을 재시작하면 이 볼륨이 내용을 못
  읽어와 빈 상태로 뜨는 경우가 있습니다 — `docker-volume-rclone` 플러그인 자체의
  알려진 문제로, 볼륨을 지웠다가 다시 만들면(위 4단계 명령을 그대로 다시 실행)
  해결됩니다.
